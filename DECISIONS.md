...
| Escalation requires verified pricing, not just a model id | `routing._escalation_ready()` requires both a non-empty `ESCALATION_MODEL` **and** a valid `Decimal` input/output entry in `PRICING_USD_PER_MILLION_TOKENS` for that exact model. Setting `ESCALATION_MODEL` alone, with no matching pricing entry, must still produce `routing.escalation_unavailable`, never a live call priced by guesswork. |
