...
| Escalation eligibility is fully explicit | Escalation requires, together: the workspace's own `gemini` key, an explicit `workspace.paid_tier_enabled` opt-in (default off), `target.fit_score >= HIGH_FIT_THRESHOLD` (85, inclusive), and `routing._escalation_ready()` (see below). Any missing condition keeps the default-tier result; the eligible-but-not-ready case is explicitly audited (`routing.escalation_unavailable`), never silent. |
| Escalation requires verified pricing, not just a model id | `routing._escalation_ready()` requires both a non-empty `ESCALATION_MODEL` **and** a `PRICING_USD_PER_MILLION_TOKENS` entry for that exact model whose `input`/`output` are each a finite, strictly positive `Decimal` — not merely present. A missing key, a non-`Decimal` value, zero, a negative rate, `NaN`, or `Infinity`/`-Infinity` in either field all leave escalation unavailable, never a live call priced by guesswork, zero, or garbage. Setting `ESCALATION_MODEL` alone, with no valid matching pricing entry, must still produce `routing.escalation_unavailable`. |
| Escalation is only promoted on a valid grounded model draft | `route_and_draft` labels the result `routing_action="escalated"` only when the escalated draft reaches `DraftStatus.LLM_OK`. A provider error or an ungrounded fallback during escalated drafting keeps the already-valid default draft/eval, records `routing.escalation_failed` with the real failure reason, and never spends an eval call judging a body that won't be used. (This is distinct from the pre-existing `INVALID_GEMINI_KEY` terminal path, checked first and unchanged.) |
...
| Generation is reserved before any provider call | `db.try_acquire_draft_generation`/`release_draft_generation` hold a durable, workspace-scoped `(workspace_id, target_id)`-unique reservation for the duration of one draft generation, acquired in `main.py`'s `create_draft` before `routing.route_and_draft` is ever called and released in a `finally` block (with a 300-second TTL as a crash backstop). This is the authoritative guard against two concurrent requests both incurring provider spend for the same target — the pre-existing `one_active_draft_per_target` unique index only ever catches the loser after both have already generated. Tenancy is enforced by the acquiring `INSERT ... SELECT ... FROM target WHERE target.workspace_id = ? AND target.id = ?` itself (the same pattern `add_draft` uses), not merely by a caller's earlier `get_target()` lookup — a target belonging to another workspace, or a nonexistent target id, matches zero rows and reserves nothing, independent of whether both ids individually exist. The reservation's own transaction is a single fast insert, never held open across a network call. |
...
| Approvals cost summary reflects all drafts, not only pending ones | `db.outreach_cost_summary` (and the Approvals template) key visibility off `draft_count > 0`, independent of whether the pending-drafts queue is currently empty; the unknown-cost "excluded" count shows whenever any exists, independent of whether any known-cost draft also exists. |
...
- `draft_generation` (Slice 6 review correction) is a durable reservation table, `(workspace_id, target_id)` unique, with an `expires_at` TTL.
